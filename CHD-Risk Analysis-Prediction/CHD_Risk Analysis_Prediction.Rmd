...
p1 <- ggplot(data = Heart, aes(x = sbp)) +
  ...
  labs(title = "Systolic Blood\nPressure (SBP)", x = "SBP (mm Hg)", y = "Count") +
  common_theme
...
mk_box <- function(var) {
  ggplot(Heart, aes(x = famhist, y = .data[[var]], fill = famhist)) +
    ...
    scale_fill_manual(values = c(Absent = "#5A9BD5", Present = "#F4A582")) +
    labs(title = var, x = "Family History (famhist)", y = NULL) +
    theme_minimal(base_size = 12) +
    theme(
      plot.title = element_text(face = "bold", hjust = 0.5, color = "#D9534F"),
